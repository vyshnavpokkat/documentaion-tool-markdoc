...
export default function MyApp({ Component, pageProps }: AppProps<MyAppProps>) {
  ...
  const [apiBoxHandle, setApiBoxHandle] = useState({ value: false, name: "doc1" });
  ...
  const doc = `{% partial file="handle.md" /%}`;
  ...
  return (
    <>
      <DataContext.Provider value={{ setApiBoxHandle }}>
        <Head>
          <title>{title}</title>
        ...
        <div className="flex">
          ...
          {apiBoxHandle.value && (
            <div className="flex-initial w-5/12 p-4 lvh overflow-auto bg-custom-mediumGray text-gray-200">
              <button
                onClick={() => setApiBoxHandle(prevState => ({ ...prevState, value: false }))}
                type="button"
                className="bg-transparent rounded-lg p-2 inline-flex items-center justify-center text-gray-400 border border-gray-400 hover:text-gray-500 hover:bg-gray-100 hover:border-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500"
              >
